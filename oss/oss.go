...
import (
	...
	"github.com/xiaomecc/utils/encrypt"
)
...
func NewOssClient(ak, sk, host, dir, callback string, expire time.Duration) *Oss {
	return &Oss{
		ak:       ak,
		sk:       sk,
		host:     host,
...
func (o *Oss) Signature() *Sign {
	t := time.Now()
	t = t.Add(time.Second * o.expire)

	dir := o.dir + t.Format("2006-01-02") + "/" + t.Format("1504")
	var callbackParam callbackParam
	callbackParam.CallbackUrl = o.callback
	...
	configStruct.Expiration = t.UTC().Format("2006-01-02T15:04:05Z")
	//condition := []string{"content-length-range", "0", "1048576000"}
	start := []string{"starts-with", "$key", dir}
	configStruct.Conditions = append(configStruct.Conditions, start)
	policy, _ := json.Marshal(configStruct)
	base64Policy := base64.StdEncoding.EncodeToString(policy)
	signature := base64.StdEncoding.EncodeToString(encrypt.HashHmac(base64Policy, o.sk))
